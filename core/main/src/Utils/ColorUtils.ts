...
import { randomInRange, mix } from "./NumberUtils";
import { midColorValue, randomColorValue } from "./Constants";
...
export function mixColors(color1: IRgb | IHsl, color2: IRgb | IHsl, size1: number, size2: number): IRgb {
...
export function getLinkColor(p1: IParticle, p2?: IParticle, linkColor?: string | IRgb): IRgb | undefined {
    if (linkColor === randomColorValue) {
        ...
        if (sourceColor && destColor && p2) {
            return mixColors(sourceColor, destColor, p1.getRadius(), p2.getRadius());
        } else {
